...
export class PostgresSchemaMapperService {
    ...
    async autoMapColumns(
        ...
        this.logger.log(
            `Auto-mapping ${sourceColumns.length} source columns to destination`,
        );
        ...
        for (const sourceCol of sourceColumns) {
            ...
            const pgType = this.mapSourceTypeToPostgres(
                sourceCol.dataType,
                sourceCol,
            );
    ...
    mapExternalTypeToPostgres(
        ...
        const mapping =
            platformMappings[sourcePlatform.toLowerCase()]?.[
    ...
    validateMapping(mapping: ColumnMapping): ValidationError[] {
        ...
        if (!mapping.nullable && !mapping.defaultValue) {
            errors.push({
                column: mapping.destinationColumn,
                error:
                    'Column is NOT NULL but has no default value. Ensure source data is always present.',
                severity: 'warning',
            });
        }
...
DO UPDATE SET ${updateCols}`;
    ...
    private findBestMatch(
        ...
        match = destinationColumns.find(
            (col) => col.name.toLowerCase() === sourceName.toLowerCase(),
        );
        if (match) return match;
     ...
     */
    private mapSourceTypeToPostgres(
        sourceType: string,
