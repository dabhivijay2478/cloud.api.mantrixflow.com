...
export class PostgresPipelineProcessor extends WorkerHost {
    ...
    async process(job: Job<PipelineJobData>): Promise<any> {
        ...
        this.logger.log(
            `Processing pipeline job ${job.id} for pipeline ${pipelineId}`,
        );
    ...
    onCompleted(job: Job<PipelineJobData>, result: any) {
        this.logger.log(
    ...
    onActive(job: Job<PipelineJobData>) {
        this.logger.log(
            `Pipeline job ${job.id} started for pipeline ${job.data.pipelineId}`,
        );
    }
}
