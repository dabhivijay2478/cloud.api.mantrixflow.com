...
export class SourceSchemaController {
  ...
  @ApiOperation({
    ...
    description: 'Get all source schemas for the organization with pagination.',
  })
  ...
  async listSourceSchemas(
    ...
    try {
      ...
      const result = await this.sourceSchemaService.findByOrganizationPaginated(
        organizationId,
        userId,
        limitNum,
        offsetNum,
      ...
      return createListResponse(result.data, `Found ${result.total} source schema(s)`, {
        total: result.total,
        limit: limitNum,
        offset: offsetNum,
