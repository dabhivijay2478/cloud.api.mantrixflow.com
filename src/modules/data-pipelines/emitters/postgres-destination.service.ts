...
export class PostgresDestinationService {
  ...
  async createDestinationTable(
    ...
    this.logger.log(`Creating destination table ${schema}.${table} with ${columns.length} columns`);
   ...
   */
  async tableExists(client: PoolClient, schema: string, table: string): Promise<boolean> {
    const query = `
      SELECT EXISTS (
      ...
      switch (writeMode) {
        ...
        case 'upsert':
          if (!upsertKey || upsertKey.length === 0) {
            throw new BadRequestException('Upsert key is required for upsert mode');
          }
          rowsWritten = await this.upsertData(client, schema, table, data, upsertKey);
          break;
        case 'replace':
  ...
  private async upsertData(
    ...
    for (let i = 0; i < data.length; i += this.BATCH_SIZE) {
      const batch = data.slice(i, i + this.BATCH_SIZE);
      const rowsWritten = await this.upsertBatch(client, schema, table, batch, upsertKey);
      totalRowsWritten += rowsWritten;
   ...
   */
  private async executeWithRetry(client: PoolClient, sql: string, params: any[]): Promise<any> {
  ...
  async addMissingColumns(
    ...
    for (const column of columns) {
      ...
      await client.query(sql);
      this.logger.log(`Added column ${column.destinationColumn} to ${schema}.${table}`);
    }
  }
   ...
   */
  async getTableStats(client: PoolClient, schema: string, table: string): Promise<TableStats> {
    const countQuery = `
      SELECT COUNT(*) as count
   ...
   */
  private generateCreateTableSQL(schema: string, table: string, columns: ColumnMapping[]): string {
    ...
    if (primaryKeyName) {
      columnDefs.push(`PRIMARY KEY (${this.quoteIdentifier(primaryKeyName)})`);
      this.logger.log(`Creating table ${schema}.${table} with primary key: ${primaryKeyName}`);
    } else {
      this.logger.warn(
