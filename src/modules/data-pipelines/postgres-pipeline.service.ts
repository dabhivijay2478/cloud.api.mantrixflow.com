...
export class PostgresPipelineService {
  ...
  async createPipeline(data: {
    ...
    if (
      ...
      !destinationTableName.startsWith('pipeline_')
    ) {
    ...
    if (data.collectors) {
      this.logger.log(`Extracting transformers from ${data.collectors.length} collectors`);
        ...
        if (collector.transformers && Array.isArray(collector.transformers)) {
          ...
          collector.transformers.forEach((transformer: any) => {
            ...
            if (transformer.fieldMappings && Array.isArray(transformer.fieldMappings)) {
              ...
              transformer.fieldMappings.forEach(
                (mapping: { source: string; destination: string }) => {
    ...
    const allTransformations = [...(data.transformations || []), ...legacyTransformations];
    ...
    try {
      ...
      const destinationSchemaExists = await this.destinationSchemaRepository.findById(
        destinationSchema.id,
      );
      ...
      if (existingPipeline && !existingPipeline.deletedAt) {
        ...
        const existingWithSchemas = await this.pipelineRepository.findByIdWithSchemas(
          existingPipeline.id,
        );
      ...
      if (verifyPipeline?.pipeline) {
        ...
        const verifyHasMappings = verifyTransformers.some(
          (t: any) =>
            t?.fieldMappings && Array.isArray(t.fieldMappings) && t.fieldMappings.length > 0,
        );
      ...
      if (
        pgErrorCode === '23503' ||
        pgErrorMessage.includes('foreign key') ||
        pgErrorConstraint?.includes('_fk')
      ) {
        throw new BadRequestException(
  ...
  async executePipeline(pipelineId: string): Promise<PipelineRunResult> {
    ...
    if (pipeline.status !== 'active') {
      throw new BadRequestException(`Pipeline is ${pipeline.status}, cannot execute`);
    }
    ...
    this.logger.log(`[${run.id}] Setting pipeline migration state to 'running'`);
    ...
    try {
      ...
      while (retryCount < maxRetries) {
        ...
        const hasValidMappings = transformers.some(
          (t: any) =>
            t?.fieldMappings && Array.isArray(t.fieldMappings) && t.fieldMappings.length > 0,
        );
      ...
      for (const transformer of transformers) {
        if (
          transformer?.fieldMappings &&
          Array.isArray(transformer.fieldMappings) &&
          transformer.fieldMappings.length > 0
        ) {
          foundFieldMappings = transformer.fieldMappings as Array<{
            source: string;
            destination: string;
            isPrimaryKey?: boolean;
          }>;
          primaryKeyField =
            transformer.primaryKeyField ||
            foundFieldMappings.find((fm) => fm.isPrimaryKey)?.destination;
          this.logger.log(
      ...
      if (foundFieldMappings.length > 0) {
        ...
        columnMappings = foundFieldMappings.map((fm) => {
          ...
          if (!primaryKeySet) {
            isPrimaryKey =
              fm.isPrimaryKey ||
              fm.destination === primaryKeyField ||
              (fm.destination.toLowerCase() === 'id' && !primaryKeyField);
            if (isPrimaryKey) {
              primaryKeySet = true;
              this.logger.log(`[${run.id}] [SETUP] Setting '${fm.destination}' as primary key`);
            }
          }

          // Check if this is an ID field (destination column is 'id' or ends with '_id')
          const isIdField =
            fm.destination.toLowerCase() === 'id' || fm.destination.toLowerCase().endsWith('_id');
      ...
      const resolvedColumnMappings = (lockedRun.resolvedColumnMappings ||
        tableResolution.columnMappings) as ColumnMapping[];
      ...
      this.logger.log(
        `[${run.id}] Step 1: Reading from source${syncCursor ? ` (cursor: ${syncCursor})` : ' (full sync)'}`,
      );
      ...
      this.logger.log(`[${run.id}] Step 2: Transforming ${sourceData.rows.length} rows`);
      ...
      this.logger.log(`[${run.id}] Using ${legacyTransformations.length} legacy transformations`);
      ...
      const hasIdInMappings = resolvedColumnMappings.some(
        (m) => m.destinationColumn.toLowerCase() === 'id',
      );
      if (hasIdInMappings && transformedData.length > 0) {
        const idMapping = resolvedColumnMappings.find(
          (m) => m.destinationColumn.toLowerCase() === 'id',
        );
        const hasIdInData =
          Object.hasOwn(transformedData[0], 'id') || Object.hasOwn(transformedData[0], 'ID');

        if (!hasIdInData && idMapping) {
          // Generate UUID for each row if ID is missing
          transformedData = transformedData.map((row, _index) => {
            // Try to get ID from source if sourceColumn exists
            const sourceId =
              idMapping.sourceColumn && row[idMapping.sourceColumn]
      ...
      this.logger.log(`[${run.id}] Step 3: Writing ${transformedData.length} rows to destination`);
      ...
      const resolvedTable = {
        ...
        columnMappings: (currentRun.resolvedColumnMappings ||
          resolvedColumnMappings) as ColumnMapping[],
      };
      ...
      if (pipeline.incrementalColumn && sourceData.rows.length > 0) {
        ...
        const isTimestampColumn =
          incrementalCol.toLowerCase().includes('_at') ||
      ...
      if (isFirstSuccessfulRun && pipeline.syncMode === 'full') {
        ...
        let detectedIncrementalColumn = timestampColumns.find((col) => sourceColumns.includes(col));
      ...
      this.logger.log(`[${run.id}] Setting pipeline migration state to 'pending' due to failure`);
  ...
  private async validateAndFixIncrementalColumn(
    ...
    try {
      ...
      try {
        ...
        if (typeResult.rows.length === 0) {
          this.logger.warn(`Column '${incrementalCol}' not found in table ${schema}.${table}`);
      ...
      this.logger.error(`Error validating incremental column: ${error.message}`, error.stack);
  ...
  private async readFromSource(
    ...
    try {
      ...
      if (sourceSchema.sourceQuery) {
        ...
        throw new BadRequestException('Either sourceQuery or sourceTable must be specified');
      }
  ...
  private async transformData(
    ...
    const transformed = sourceData.map((row, rowIndex) => {
      ...
      mappings.forEach((mapping) => {
        ...
        if (
          ...
          mapping.defaultValue === 'gen_random_uuid()' &&
          !value
        ) {
      ...
      if (rowIndex === 0) {
        this.logger.log(`First transformed row keys: ${Object.keys(transformedRow).join(', ')}`);
      }
    ...
    if (transformed.length > 0 && Object.keys(transformed[0]).length === 0) {
      ...
      throw new BadRequestException('Transformed data is empty. Please check your field mappings.');
    }
  ...
  private async resolveAndPrepareDestinationTable(
    ...
    try {
      ...
      this.logger.log(`${logPrefix} [TRANSACTION] Started transaction for table resolution`);

      // Step 1: Resolve column mappings (field mappings are the SINGLE source of truth)
      const finalColumnMappings = columnMappings || destinationSchema.columnMappings || [];
      ...
      const hasIdField = finalColumnMappings.some(
        (m) => m.destinationColumn.toLowerCase() === 'id',
      );
      if (!hasIdField) {
      ...
      this.logger.log(`${logPrefix} [TRANSACTION] Committed transaction for table resolution`);
  ...
  private async writeToDestination(
    ...
    let writeMode: 'append' | 'upsert' | 'replace' =
  ...
  async validatePipeline(pipelineId: string): Promise<ValidationResult> {
    ...
    if (
      writeMode === 'upsert' &&
      (!destinationSchema.upsertKey || destinationSchema.upsertKey.length === 0)
    ) {
   ...
   */
  async togglePipeline(pipelineId: string, status: 'active' | 'paused'): Promise<void> {
   ...
   */
  async deletePipeline(pipelineId: string, dropTable: boolean = false): Promise<void> {
    const pipelineWithSchemas = await this.pipelineRepository.findByIdWithSchemas(pipelineId);
    if (!pipelineWithSchemas) {
      throw new NotFoundException(`Pipeline ${pipelineId} not found`);
    }
